...
Namespace Async
    ...
    Public Class Saml_tests
        ...
        Public Async Function Getting_sso_initiation_endpoint(clientBuilder As TestClientProvider) As Task
            Dim client = clientBuilder.GetClient()
            Dim defaultApplication = Await client.GetApplications() _
                .Where(Function(x) x.Name = "My Application") _
                .SingleOrDefaultAsync()

            Dim samlPolicy = Await defaultApplication.GetSamlPolicyAsync()
            samlPolicy.Href.ShouldNotBeNullOrEmpty()

            Dim samlProvider = Await samlPolicy.GetSamlServiceProviderAsync()
            samlProvider.Href.ShouldNotBeNullOrEmpty()

            Dim ssoInitiationEndpoint = Await samlProvider.GetSsoInitiationEndpointAsync()
